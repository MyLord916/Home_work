...
def state_poling():
    ...
    if state == 'start':
        ...
        #        print(rd_num)  #Подсказка для отладки
        print(f'\nЗагаданно число от 1 до {set_num}, попробуй отгадать\n')
        state = 'game'
        return
    elif state == 'game':
        answer = input()
        if answer.isdigit() and 1 <= int(answer) <= set_num:
            ...
            print('\nМожете настроить парраметры игры, укажите предел загаданного числа:', end=' ')
            state = 'settings'
            return
        else:
